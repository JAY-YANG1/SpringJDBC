...
public class SungJukDAOImpl implements SungJukDAO {
	...
	public List<SungJuk> selectAllSungJuk() {
		...
		// 결과집합이 존재하면(rs.next 가 참인 경우) 
		// IoC 컨테이너가 mapper 객체의 mapRow 를 호출함
	...
	public void updateSungJuk(SungJuk sj) {
		String sql = "update sungjuk set kor = ?, eng = ?, mat = ?, tot = ?, mean = ?, grd = ? " 
					+ "where sjno = ?";
		
		Object[] params = new Object[] {
			sj.getKor(), sj.getEng(), sj.getMat(), sj.getTot(),
			sj.getAvg(), sj.getGrd()+"", sj.getSjno()
		};
